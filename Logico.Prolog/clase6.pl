...
grave(narcotrafico(Drogas)) :- length(Drogas,Tam) , Tam >=5.

/* OTRA FORMA EN QUE LO HICE
peligroso(Preso) :-
    prisionero(Preso,homicidio(_)),
    not(prisionero(Preso,robo(_))),
    not(narcotraficoGrave(Preso)).

peligroso(Preso) :-
    narcotraficoGrave(Preso),
    not(prisionero(Preso,robo(_))).

narcotraficoGrave(Preso) :-
    prisionero(Preso,narcotrafico(Drogas)),
    length(Drogas,Tam) , Tam >=5.
*/
...
condena(Preso,Condena) :-
    ...
    sumlist(Penas,Condena). %SUMLIST ES INVERSIBLE
